...
const {
  ...
  NODE_ENV, MAX_PLAYERS, GRAVITY, JUMP_VELOCITY, DINO_SPEED,
...
export const defaultServerConfig: ServerConfig = {
  ...
  gameConfig: {
    ...
    // Obstacle dimensions
    cactusWidth: parseInt(process.env[CACTUS_WIDTH] ?? '30', 10),
    cactusHeight: parseInt(process.env[CACTUS_HEIGHT] ?? '50', 10),
    birdWidth: parseInt(process.env[BIRD_WIDTH] ?? '40', 10),
    birdHeight: parseInt(process.env[BIRD_HEIGHT] ?? '30', 10),

    // Spawn positions
    obstacleSpawnX: parseInt(process.env[OBSTACLE_SPAWN_X] ?? '800', 10),
    birdSpawnY: parseInt(process.env[BIRD_SPAWN_Y] ?? '100', 10),
  },
};
